...
class WsOrderbookCache:
    ...
    def get_depth(self, exchange: str, symbol: str) -> Optional[Dict[str, list]]:
        """Return full orderbook depth from WS feed.

        Returns {"bids": [[price, qty], ...], "asks": [[price, qty], ...]}
        or None if stale/unavailable.
        """
        # Use snapshot staleness check — depth and snapshot are updated atomically
        snapshot = self._orderbooks.get(exchange, {}).get(symbol)
        if not snapshot:
            return None
        if time.time() - snapshot.timestamp > self._stale_after_sec:
            return None
        return self._depth.get(exchange, {}).get(symbol)
